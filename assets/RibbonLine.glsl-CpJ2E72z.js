...
return worldToScreenPerDistanceRatio / segmentDistanceToCamera;
  ...
  `),n.uniforms.add(new si("stipplePatternTexture",r=>r.stippleTexture),new Y("stipplePatternPixelSizeInv",r=>1/wt(r))),e.stippleOffColorEnabled&&n.uniforms.add(new Ne("stippleOffColor",r=>fa(r.stippleOffColor))),t.include(Je),e.worldSizedImagePattern?(t.varyings.add("vStippleV","float"),t.fragment.include(Ni),n.code.add(s`vec4 getStippleColor(out bool isClamped) {
...
float stippleAlpha = step(alphaCutoff, stippleCoverage);`):r.main.add(s`float stippleAlpha = getStippleAlpha(lineWidth);`),o!==11&&r.main.add(s`discardByStippleAlpha(stippleAlpha, alphaCutoff);`),e.include(Je),r.uniforms.add(new Ne("intrinsicColor",u=>u.color)).main.add(s`vec4 color = intrinsicColor * vColor;
color.a = noPerspectiveRead(color.a);`),h&&r.uniforms.add(new Ne("innerColor",u=>u.innerColor??u.color),new Y("innerWidth",(u,$)=>u.innerWidth*$.camera.pixelRatio)).main.add(s`float distToInner = abs(lineDistance) - innerWidth;
...
finalColor.a *= 1.0 - clamp(feather, 0.0, 1.0);`),k&&r.main.add(s`
